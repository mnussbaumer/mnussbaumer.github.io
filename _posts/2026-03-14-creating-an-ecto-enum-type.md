...
slug: creating-an-ecto-enum-type
...
[in portuguese](/posts/criar-um-ecto-type-como-enum)
...
In this case, we basically defined as valid values only that specific set of atoms and so when casting against that schema field, any string value, or atom, that matches those atoms if converted would also be a valid value.
